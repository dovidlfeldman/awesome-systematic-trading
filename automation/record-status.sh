...
# reaches the remote.
#
# Usage: record-status.sh <cycle|check> <claude_status> <mirror_status>
#        where each status is the literal string "ok" or "FAILED".
set -euo pipefail
...
printf '%s\t%s\tclaude=%s\tmirror=%s\tmd_repo=%s\tmd_mirror=%s\tcommit=%s\n' \
  "$(date -u +%FT%TZ)" "$KIND" "$RUN" "$MIRROR" "$REPO_MD" "$MIRROR_MD" \
  "$(git rev-parse --short HEAD)" >>"$STATUS_FILE"

git add "$STATUS_FILE"
git commit -q -m "Run status: $KIND claude=$RUN mirror=$MIRROR ($REPO_MD/$MIRROR_MD md)"
git push -q origin "$(git rev-parse --abbrev-ref HEAD)"

echo "Recorded status -> automation/run-status.tsv (claude=$RUN mirror=$MIRROR, $REPO_MD/$MIRROR_MD md)"
