...
NOTE_PREFIX="$RUN_DATE $RUN_SLOT Cycle"
...
- Run kind: **daily cycle (full rotation)**
- Date (ET): **$RUN_DATE**
- Slot (ET): **$RUN_SLOT**
- **Required run note.** Create exactly one file whose name starts with
  ...
  prefix (e.g. \`trading-vault/Trades/$NOTE_PREFIX — No Trades.md\`). Write it even if nothing
  traded, and even if you skip this run. If you do not, the runner writes a failure stub in
  its place.
"

if claude -p "$(cat "$REPO_DIR/automation/daily-cycle-prompt.md")$RUN_CONTEXT" \
...
set +e
"$REPO_DIR/automation/ensure-run-note.sh" cycle "$RUN_DATE" "$RUN_SLOT" "$RUN" "$LOG_FILE" >>"$LOG_FILE" 2>&1
...
"$REPO_DIR/automation/record-status.sh" cycle "$RUN" "$MIRROR" "$NOTE" >>"$LOG_FILE" 2>&1 \
